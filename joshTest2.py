from flat_game import carmunkExperiment2
import numpy as np
import random
import csv
import os.path
...
def writeActionMap(game_state, fileObj):
    ...
    for r in range(4):
        for c in range(10):
            fileObj.write("[")
            for a in range(4):
                actionValue = "{:.4f}".format(cellList[r][c].actions[a])
...
def learn():
    ...
    totalReward = 0.0
    fileObj = open("experiment2.csv", "w")

    # Create a new game instance.
    game_state = carmunkExperiment2.GameState()
    ...
    while e < episodes:
        ...
        #end while
        #Decrement epsilon over time to reduce the exploration.
        if (e % observe == 0):
             epsilon -= 0.01
