...
public class RLPractice {
	public static void main(String [] args){
		SumoTraciConnection conn = new SumoTraciConnection(
				"test/resources/sumo_maps/RLSUMOPractice/rl.sumo.cfg",  // config file
				12345                                  // random seed
				);
		try{
			conn.runServer();
			conn.nextSimStep();
			int num = 0;
			java.awt.Color g = java.awt.Color.GREEN;
			int timeTotal = 1000;
			for(int i = 0; i < timeTotal; i++){
				...
				//System.out.println("Current Speed: " + veh.getSpeed() + " Colour: " + veh.getColor().toString());
				if(vehicles.iterator().hasNext()){
					Vehicle veh = vehicles.iterator().next();
					
					if(num == 0){
						veh.changeColor(g);
						veh.changeSpeed(10.00);
						num++;
					}
					
					
					System.out.println("Current Speed: " + veh.getSpeed() + " Colour: " + g.toString());
					
					if(time % 50 == 0){
						//veh.changeSpeed(100.00);
						//System.out.println("Current Speed: " + veh.getSpeed());
